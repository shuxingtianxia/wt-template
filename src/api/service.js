...
function createService() {
  ...
  service.interceptors.request.use(
    config => {
      config.headers['language'] = util.cookies.get('lang') && util.cookies.get('lang').toUpperCase()
  ...
  service.interceptors.response.use(
    response => {
      // 判断返回的是不是文件流，如果是文件流，直接把data值返出去
      // const bool = fileStream.some(item => response.config.url.includes(item))
      // if (bool) {
