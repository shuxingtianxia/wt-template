export const buttonArr = [
  {
    icon: 'el-icon-plus',
    text: '保存',
    type: 'primary',
    clickType: 'add',
    code: 'info-save'
  },
  {
    icon: 'el-icon-edit',
    text: '返回',
    clickType: 'back'
  }
]
export const fields = [
  {
    type: 'select',
    label: '系统代码',
    prop: 'systemCode',
    span: 8,
    rules: [{ required: true, message: '请选择系统代码', trigger: 'change' }]
  },
  {
    type: 'select',
    label: '系统所属项目群',
    prop: 'systemType',
    span: 8,
    rules: [{ required: true, message: '请选择系统所属项目群', trigger: 'change' }]
  },
  {
    type: 'select',
    label: '项目类别',
    prop: 'type',
    span: 8,
    rules: [{ required: true, message: `请选择项目类别`, trigger: 'change' }]
  },
  {
    type: 'input',
    label: '项目名称',
    prop: 'name',
    span: 8,
    maxLength: 64,
    rules: [{ required: true, message: `请输入项目名称`, trigger: 'blur' }]
  },
  {
    type: 'select',
    label: '项目开发语言',
    prop: 'language',
    span: 8,
    multiple: true,
    rules: [{ required: true, message: '请选择项目开发语言', trigger: 'blur' }]
  },
  {
    type: 'input',
    label: '项目版本',
    prop: 'version',
    span: 8,
    maxLength: 64,
    rules: [{ required: true, message: '请输入项目版本', trigger: 'blur' }]
  },
  {
    label: '项目阶段',
    prop: 'stage',
    span: 8,
    rules: [{ required: false, message: '请选择项目阶段', trigger: 'change' }]
  },
  {
    type: 'select',
    label: '是否延期',
    prop: 'delay',
    span: 8,
    rules: [{ required: true, message: '请选择是否延期', trigger: 'change' }]
  },
  {
    label: '项目状态',
    prop: 'state',
    span: 8,
    rules: [{ required: false, message: '请选择项目状态', trigger: 'change' }]
  },
  {
    type: 'select',
    label: '项目领域',
    prop: 'field',
    span: 8,
    rules: [{ required: true, message: '请选择项目领域', trigger: 'change' }]
  },
  {
    type: 'textarea',
    label: '项目介绍',
    prop: 'description',
    span: 8,
    maxLength: 64,
    rules: [{ required: true, message: '请输入项目介绍', trigger: 'blur' }]
  }
]
export const fieldsInfo = [
  {
    type: 'input',
    label: '项目人天',
    prop: 'dayCause',
    span: 8,
    maxLength: 64,
    placeholder: '请输入人天，例如:50',
    rules: [{ required: true, message: '请输入项目人天', trigger: 'blur' }]
  },
  {
    type: 'select',
    label: '项目等级',
    prop: 'level',
    span: 8,
    rules: [{ required: true, message: '请选择项目等级', trigger: 'change' }]
  }
]
export const fieldsAssessment = [
  {
    type: 'select',
    label: '是否参与考核',
    prop: 'assessmentState',
    rules: [{ required: true, message: '请选择是否参与考核', trigger: 'change' }]
  },
  {
    type: 'input',
    label: 'SVN文档地址',
    prop: 'svnUrl',
    span: 10,
    maxLength: 64,
    placeholder: '请输入SVN文档地址',
    rules: [{ required: true, message: '请输入SVN文档地址', trigger: 'blur' }]
  }
]
export const rrAttachmentFields = [
  {
    type: 'datetime',
    label: '需求评审日期',
    prop: 'demandReviewDate',
    rules: [{ required: true, message: '请选择需求评审日期', trigger: 'change' }]
  },
  {
    label: '项目文档',
    prop: 'rrRadioOne',
    span: 24
  }
]
export const prAttachmentFields = [
  {
    type: 'datetime',
    label: '立项日期',
    prop: 'projectDate',
    rules: [{ required: true, message: '请选择立项日期', trigger: 'change' }]
  },
  {
    label: '项目文档',
    prop: 'prRadioOne',
    span: 24
  }
]
export const srAttachmentFields = [
  {
    type: 'datetime',
    label: '计划日期',
    prop: 'schemeReviewPlanDate',
    rules: [{ required: true, message: '请选择计划日期', trigger: 'change' }]
  },
  {
    type: 'datetime',
    label: '实际日期',
    prop: 'schemeReviewActualDate',
    rules: [{ required: false, message: '请选择实际日期', trigger: 'change' }]
  },
  {
    label: '项目文档',
    prop: 'srRadioOne',
    span: 24
  }
]
export const dpAttachmentFields = [
  {
    type: 'datetime',
    label: '计划日期',
    prop: 'planDate',
    rules: [{ required: true, message: '请选择计划日期', trigger: 'change' }]
  },
  {
    type: 'datetime',
    label: '实际日期',
    prop: 'planActualDate',
    rules: [{ required: false, message: '请选择实际日期', trigger: 'change' }]
  },
  {
    label: '项目文档',
    prop: 'dpRadioOne',
    span: 24
  }
]
export const qasTestAttachmentFields = [
  {
    type: 'datetime',
    label: '计划日期',
    prop: 'qasPlanDate',
    rules: [{ required: true, message: '请选择计划日期', trigger: 'change' }]
  },
  {
    type: 'datetime',
    label: '实际日期',
    prop: 'qasActualDate',
    rules: [{ required: false, message: '请选择实际日期', trigger: 'change' }]
  },
  {
    label: '项目文档',
    prop: 'qasRadioOne',
    span: 24
  }
]
export const uatTestAttachmentFields = [
  {
    type: 'datetime',
    label: '计划日期',
    prop: 'uatPlanDate',
    rules: [{ required: true, message: '请选择计划日期', trigger: 'change' }]
  },
  {
    type: 'datetime',
    label: '实际日期',
    prop: 'uatActualDate',
    rules: [{ required: false, message: '请选择实际日期', trigger: 'change' }]
  },
  {
    label: '项目文档',
    prop: 'uatRadioOne',
    span: 24
  }
]
export const lrAttachmentFields = [
  {
    type: 'datetime',
    label: '计划日期',
    prop: 'linePlanDate',
    rules: [{ required: true, message: '请选择计划日期', trigger: 'change' }]
  },
  {
    type: 'datetime',
    label: '实际日期',
    prop: 'lineActualDate',
    rules: [{ required: false, message: '请选择实际日期', trigger: 'change' }]
  },
  {
    label: '项目文档',
    prop: 'lrRadioOne',
    span: 24
  }
]
