...
export default {
  ...
  actions: {
     ...
     */
    async open({ state, commit, dispatch }, { name, params, query, fullPath, meta, matched }) {
      // 已经打开的页面
      ...
      if (isKeepAlive({ meta })) {
        if (matched.length >= 3) {
          for (let i = 1; i < matched.length; i++) {
            commit('keepAlivePush', matched[i].name)
          }
        } else {
     ...
     */
    async close({ state, commit, dispatch }, { tagName, isTo }) {
      // 预定下个新页面
      let newPage = {}
